...
export function calculateNgocHanhCuc(menhGridIdx, yearCanIdx) {
  ...
  if (napAmMenh.includes("Thủy")) {
    return { name: "Thủy Nhị Cục", num: 2, element: "Thủy" };
  } else if (napAmMenh.includes("Mộc")) {
    return { name: "Mộc Tam Cục", num: 3, element: "Mộc" };
  } else if (napAmMenh.includes("Kim")) {
    return { name: "Kim Tứ Cục", num: 4, element: "Kim" };
  } else if (napAmMenh.includes("Thổ")) {
    return { name: "Thổ Ngũ Cục", num: 5, element: "Thổ" };
  } else {
    return { name: "Hỏa Lục Cục", num: 6, element: "Hỏa" };
  }
}
...
export function generateTuViChart(data) {
  ...
  tuViGroup.forEach(s => grid[(tuViIdx + s.offset + 1200) % 12].chinhTinh.push(s));
  ...
  thienPhuGroup.forEach(s => grid[(thienPhuIdx + s.offset) % 12].chinhTinh.push(s));
