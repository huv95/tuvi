...
// ===== Hạn của một năm xem ====================================================
// Trả về cung Đại Hạn và cung Tiểu Hạn ứng với năm xem, tính từ lá số đã an.
// Hàm thuần, không chạm DOM — dùng lại được ở Node và ở trang xem hạn.
//
//   grid        mảng 12 cung đã có palaceName/chiName/tieuHan (từ generateTuViChart)
//   lunarYear   năm sinh ÂM lịch (tuổi tử vi đếm theo năm âm, không theo dương)
//   viewYear    năm âm lịch muốn xem hạn
//   viewYearChi Chi của năm xem
//   menhIdx     chỉ số lưới cung Mệnh · cucNum số Cục (2..6) · isThuanLy chiều đi
//
// Tuổi dùng ở đây là tuổi âm (tuổi mụ): sinh ra đã là 1 tuổi, mỗi Tết thêm 1
// → tuoi = viewYear - lunarYear + 1.
//
// Đại Hạn: hạn đầu khởi tại cung Mệnh đúng năm `cucNum` tuổi, mỗi cung 10 năm,
// đi thuận nếu thuận lý / nghịch nếu nghịch lý. Sau 120 năm thì lặp lại vòng
// mới (`vong` = 2 trở lên), tuổi vẫn tính tiếp chứ không quay về đầu.
//
// Tiểu Hạn: mỗi cung đã mang sẵn Chi của năm mà tiểu hạn ghé qua (an ở
// generateTuViChart theo tam hợp Chi năm sinh, nam thuận nữ nghịch), nên chỉ
// cần tìm cung có Chi trùng Chi năm xem — chu kỳ 12 năm nên luôn có đúng một.
//
// Lưu Niên Đại Hạn: lộ trình 10 năm BÊN TRONG một đại vận, sang đại vận khác
// thì lập lại lộ trình mới (khác tiểu hạn — tiểu hạn là một bảng cố định cả
// đời). Bốn chặng, đếm theo CHIỀU ĐI CỦA ĐẠI HẠN (thuận lý đi thuận, nghịch
// lý đi nghịch), gọi d = +1 / -1 và G = cung đại hạn:
//
//   năm 1        G                 đứng ngay cung đại vận ("Gốc")
//   năm 2        G + 6             cung xung chiếu
//   năm 3        G + 6 - d         lùi 1 cung từ cung xung chiếu
//   năm 4..10    G + 6 + (k-4)d    tiến liên tiếp, mỗi năm một cung
//
// Hệ quả tự kiểm: năm thứ 10 luôn quay về đúng G, nên bước tiếp theo (tuổi đầu
// của đại vận sau) rơi đúng vào cung đại vận kế tiếp — lộ trình cũ giao lại
// cho lộ trình mới, không hở. Test mục F kiểm tính chất này trên mọi lá số.
// Lưu ý theo đúng khẩu quyết: năm 4 trở lại cung của năm 2, và trong 10 năm
// lộ trình chỉ đi qua 8 cung khác nhau.
//
// Nguyệt Hạn (lưu Nguyệt) — phái Đẩu Số, nguồn refs/dau-so-tinh-thanh/
// chuong-10-van-the-hau-thien-nien-han.md mục 134: "Lấy cung Đấu quân an tháng
// giêng, còn lại các lưu Nguyệt thuận bàn mà đẩy, một tháng một cung", với Đẩu
// Quân tính theo Chi NĂM XEM (xem viTriDauQuan ở trên). Vì Đẩu Quân dịch một
// cung mỗi năm nên lộ trình 12 tháng cũng xoay theo năm.
// Chưa xử lý tháng nhuận: tháng nhuận dùng chung cung với tháng chính. Cách
// chia nửa tháng nhuận sang cung kế tiếp còn tranh chấp giữa các phái —
// refs/tu-vi-tong-hop/quyen2-01-thuat-giai-doan.md mục 3 nói rõ điều đó.
// Tử Vi Giảng Minh (phan-3-chuong-05-van.md dòng 636) dùng phép khác: tháng
// Giêng cố định ở cung Dần, tháng 2 ở Mão... — không cài, chỉ ghi để biết.
// Vị trí Đẩu Quân (chỉ số lưới): từ cung Thái Tuế của năm đang xét an tháng
// Giêng, đếm NGHỊCH tới tháng sinh, rồi từ đó đếm THUẬN tới giờ sinh.
// Nguồn: refs/dau-so-tinh-thanh/chuong-10-van-the-hau-thien-nien-han.md mục 134.
// Dùng hai chỗ: an sao Đẩu Quân trên lá số (lấy Chi năm sinh) và tìm cung tháng
// Giêng của nguyệt hạn một năm bất kỳ (lấy Chi năm xem) — cùng một công thức,
// đúng như sách: "Đấu quân năm sinh định ra, về sau từng năm Đấu quân thuận bàn
// mà đẩy, một năm một cung".
export function viTriDauQuan(chiNamIdx, lunarMonth, hourChiIdx) {
  return (chiNamIdx - 2 - (lunarMonth - 1) + hourChiIdx + 120) % 12;
}

export function tinhHan(grid, { lunarYear, viewYear, viewYearChi, viewYearChiIdx,
                               lunarMonth, hourChiIdx, viewMonth,
                               menhIdx, cucNum, isThuanLy }) {
  const tuoi = viewYear - lunarYear + 1;
  const o = (gridIdx) => ({
    gridIdx,
    chiName: grid[gridIdx].chiName,
    palaceName: grid[gridIdx].palaceName,
  });

  if (tuoi < 1) {
    return { viewYear, tuoi, daiHan: null, tieuHan: null, luuNienDaiHan: null, nguyetHan: null,
             ghiChu: `Năm ${viewYear} trước năm sinh (${lunarYear} âm lịch) — chưa có hạn` };
  }

  let daiHan = null, ghiChu = '';
  if (tuoi < cucNum) {
    // Trước tuổi khởi Đại Hạn: các sách gọi là giai đoạn "hạn ấu thơ", chưa
    // vào Đại Hạn nào — không gán bừa vào cung Mệnh.
    ghiChu = `Chưa vào Đại Hạn đầu — ${cucNum} tuổi mới khởi hạn tại cung Mệnh`;
  } else {
    const i = Math.floor((tuoi - cucNum) / 10);      // đã qua bao nhiêu đại hạn
    const step = isThuanLy ? i % 12 : (12 - i % 12) % 12;
    const tuTuoi = cucNum + i * 10;
    daiHan = {
      ...o((menhIdx + step) % 12),
      tuTuoi, denTuoi: tuTuoi + 9,
      tuNam: lunarYear + tuTuoi - 1, denNam: lunarYear + tuTuoi + 8,
      thuTu: i % 12 + 1,                             // đại hạn thứ mấy trong vòng
      vong: Math.floor(i / 12) + 1,
    };
  }

  let luuNienDaiHan = null;
  if (daiHan) {
    const d = isThuanLy ? 1 : -1;
    const buoc = (k) => k === 1 ? 0 : k === 2 ? 6 : 6 + (k - 4) * d;
    const loTrinh = [];
    for (let k = 1; k <= 10; k++) {
      loTrinh.push({
        namThu: k,
        tuoi: daiHan.tuTuoi + k - 1,
        nam: daiHan.tuNam + k - 1,
        ...o((daiHan.gridIdx + buoc(k) + 120) % 12),
      });
    }
    luuNienDaiHan = { ...loTrinh[tuoi - daiHan.tuTuoi], loTrinh };
  }

  // Nguyệt hạn: tháng Giêng tại Đẩu Quân của năm xem, mỗi tháng thuận một cung.
  const dauQuanIdx = viTriDauQuan(viewYearChiIdx, lunarMonth, hourChiIdx);
  const loTrinhThang = [];
  for (let t = 1; t <= 12; t++) loTrinhThang.push({ thang: t, ...o((dauQuanIdx + t - 1) % 12) });
  const thangXem = viewMonth >= 1 && viewMonth <= 12 ? viewMonth : null;
  const nguyetHan = {
    dauQuan: o(dauQuanIdx),
    thangXem,
    ...(thangXem ? o(loTrinhThang[thangXem - 1].gridIdx) : {}),
    loTrinh: loTrinhThang,
  };

  const tieuHanIdx = grid.findIndex(c => c.tieuHan === viewYearChi);
  return { viewYear, tuoi, daiHan, tieuHan: tieuHanIdx === -1 ? null : o(tieuHanIdx),
           luuNienDaiHan, nguyetHan, ghiChu };
}

export function generateTuViChart(data) {
  let { name, gender, isSolar, day, month, year, hour, minute, viewYear, viewMonth, isLeapMonth } = data;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    grid.push({
      ...
      isDaiHan: false,        // cung Đại Hạn của năm xem — gán ở phần tính hạn cuối hàm
      isTieuHan: false,       // cung Tiểu Hạn của năm xem
      isLuuNienDaiHan: false, // cung Lưu Niên Đại Hạn của năm xem
      isNguyetHan: false      // cung Nguyệt Hạn của tháng xem (chỉ khi truyền viewMonth)
    });
  }
  ...
  push(viTriDauQuan(yChi, lMonth, hChi), "Đẩu Quân");  // Thái Tuế -> nghịch tháng -> thuận giờ
  ...
  let han = tinhHan(grid, { lunarYear: lunar.year, viewYear, viewYearChi: viewYearCanChi.chi,
                            viewYearChiIdx: viewYearCanChi.chiIdx,
                            lunarMonth: lunar.month, hourChiIdx: hourCanChi.chiIdx, viewMonth,
                            menhIdx, cucNum: cuc.num, isThuanLy });
  if (han.daiHan) grid[han.daiHan.gridIdx].isDaiHan = true;
  if (han.tieuHan) grid[han.tieuHan.gridIdx].isTieuHan = true;
  if (han.luuNienDaiHan) grid[han.luuNienDaiHan.gridIdx].isLuuNienDaiHan = true;
  if (han.nguyetHan && han.nguyetHan.thangXem) grid[han.nguyetHan.gridIdx].isNguyetHan = true;
